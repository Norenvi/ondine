...
│   │   ├── CommunePanel.tsx    # Coquille du panneau carte au clic/recherche (Paper + header collant) autour de CommuneMesures variant="compact"
...
│   │   ├── Leaderboard.tsx     # Vue plein ecran depuis la TopBar : classement des communes (DataGrid), clic ligne -> drill CommuneMesures variant="full", "Voir sur la carte" + TimelineSlider en pied
...
- `CommunePanel.tsx` : coquille (Paper positionné côté carte + en-tête collant nom/fermer) autour de `CommuneMesures variant="compact"`.
- `Leaderboard.tsx` : vue **plein écran** (Paper `inset:0`, ouverte depuis la TopBar). Navigateur à deux niveaux : DataGrid de toutes les communes classées (reste monté quand on drille, donc tri/filtre/page préservés), clic sur une ligne -> `drilledCode` -> `CommuneMesures variant="full"` par-dessus, avec fil d'Ariane retour + bouton "Voir sur la carte" (`handleDrillIntoCommune` : cale le niveau carte sur commune, ferme le classement). `TimelineSlider` en pied de vue (le slider carte est masqué par le plein écran).
...
**`Leaderboard.tsx` est devenu un navigateur de données à deux niveaux** (septembre 2026) : plein écran depuis la TopBar, classement des communes -> drill sur une commune (relevés via `CommuneMesures variant="full"`), grille de classement conservée montée (état tri/filtre préservé), `TimelineSlider` en pied. Reste par paramètre (un seul `apiCode` à la fois).
