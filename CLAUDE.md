...
│   │   ├── models.py           # ORM : region/departement/epci/commune/parametre/reseau (+ distributeur/maitre_ouvrage)/mesure + commune_valeur (cache d'agrégation)
│   │   ├── deps.py             # get_parametre (resout ?parametre=code -> objet Parametre ou 404)
│   │   ├── schemas.py          # Pydantic : ParametreOut, AggregationOut, MesureOut (date/valeur/reseau/distributeur/conclusion/valeur_libelle), CommuneOut
│   │   └── routers/
...
│   │       └── communes.py     # GET /communes/{code_insee}, /communes/{code_insee}/mesures?parametre=code[&annee=YYYY], /communes/{code_insee}/bulletin?annee=YYYY (résumé par paramètre : min/max/moyenne/dernier lus depuis mesure, pas le cache commune_valeur)
...
│   │   ├── CommunePanel.tsx    # Panneau carte au clic/recherche (Paper flex-column, header nom/fermer) autour de CommuneDetail variant="compact". Passe onParameterChange=handleParameterChange : ouvrir une ligne du Bulletin recolore la carte sur ce paramètre
│   │   ├── CommuneDetail.tsx   # Corps partagé CommunePanel + drill Leaderboard : Tabs "<paramètre actif>" (CommuneMesures) / "Bulletin", colonne flex (tabs fixes, contenu scrollable = conteneur du sticky header du Bulletin). Ouvre sur l'onglet paramètre
│   │   ├── CommuneMesures.tsx  # Bloc partagé : resume (moyenne/taux) + DataGrid/graphe des releves d'une commune. variant "compact" (carte) vs "full" (Leaderboard, +classe +distributeur)
│   │   ├── Bulletin.tsx        # Tableau (groupé par PARAMETER_GROUPS) des paramètres mesurés dans la commune pour l'année, colonnes Plage/Moyenne/Limite (N/A si pas de seuil)/Dernier + chip Classe (seule Moyenne et Dernier sont alignées à droite, le reste à gauche) + alerte rouge si un relevé hors limite. La colonne Moyenne et le chip = la MÊME moyenne poolée que le choropleth (cohérence carte/bulletin/CommuneMesures) ; Plage/Dernier montrent la dispersion. Chip = taux de non-conf pour un paramètre à seuil. Clic sur une ligne -> Collapse in-place vers `CommuneMesures` + auto-scroll + `onParameterChange(id)` (le host recolore la carte). `initialExpanded` ouvre+scrolle une ligne (suivi via un effet dédié, hors deps du fetch). variant "compact" / "full"
│   │   ├── Leaderboard.tsx     # Vue plein ecran depuis la TopBar : classement des communes (DataGrid), clic ligne -> drill `CommuneDetail variant="full"` (onglets paramètre/Bulletin), "Voir sur la carte" + TimelineSlider en pied
│   │   ├── Legend.tsx          # Legende (classes + plages du parametre actif) + switch unite + switch theme
│   │   ├── Timeline.tsx        # TimelineSlider (slider année nu, snap only) + Timeline (wrapper flottant carte). Masqué si < 2 années
│   │   ├── parameters.ts       # Registre PARAMETERS (29 params) : classes de couleur, unites, REGULATORY_LIMITS (seuils reglementaires par param), expression fill-color
...
│   │   ├── entities.ts        # Index generique par niveau (code/nom/bbox) depuis le GeoJSON, socle de communes.ts
...
│   │   └── urlState.ts         # Permaliens : lecture/écriture de l'état partageable dans la query string (parametre/annee/unite/niveau/entite/classement), pas de router. App le lit 1x au montage et fait replaceState à chaque changement
...
- **sqlalchemy** + **psycopg** : seed de la base Postgres depuis `seed_db.py` (importe les modèles ORM directement depuis `backend/src/models.py` via `sys.path.insert`, pour ne pas dupliquer le schéma). **L'image `pipeline` COPY `backend/src/` au build** (pas de bind mount) : après un changement de `models.py`, `docker compose --profile tools build pipeline` avant de re-seeder, sinon `AttributeError` sur une colonne ajoutée (déjà rencontré avec `reseau.distributeur`).
- `seed_db.load_reseaux(com_udi, plv)` : dérive `distributeur`/`maitre_ouvrage` = valeur **modale** de `DIS_PLV.distrlib`/`moalib` par `cdreseau` (~13 % des réseaux en ont plusieurs sur une année). Upsert `on_conflict_do_update` + `COALESCE`, `years` trié croissant -> l'archive la plus récente gagne. NULL pour les réseaux absents de PLV.
- `seed_db.derive_valeur_libelle(rqana)` : garde la chaîne `rqana` seulement si elle n'est pas un nombre nu (`<0,5`, `>100`, `N.M.`, `traces`), NULL sinon -> la colonne ne grossit que pour la minorité qualifiée.
...
- Sources de données :
  - Hub'Eau, dataset "Résultats du contrôle sanitaire de l'eau distribuée commune par commune" (data.gouv.fr), archive nationale `dis-{année}.zip`. Contient 3 fichiers CSV UTF-8 : `DIS_RESULT_{année}.txt` (~938 Mo, colonnes utiles `referenceprel`/`cdparametre`/`cdunitereferencesiseeaux`/`valtraduite`/`rqana`), `DIS_PLV_{année}.txt` (~51 Mo, `referenceprel`/`cdreseau`/`dateprel`/`inseecommuneprinc`/`conclusionprel`/`distrlib`/`moalib`), `DIS_COM_UDI_{année}.txt` (~4 Mo). Jointure RESULT → PLV via `referenceprel`, PLV → COM_UDI via `cdreseau` pour obtenir `inseecommune`.
...
### Backend (FastAPI)
- **Modèle relationnel** (`backend/src/models.py`) : `region` / `departement` / `epci` / `commune` (hiérarchie admin, sans géométrie) + `parametre` (catalogue, `code`/`cdparametre_sandre`/`unite`) + `reseau` (UDI, + `distributeur`/`maitre_ouvrage` nullables = `DIS_PLV.distrlib`/`moalib` modaux par cdreseau, sur `reseau` et pas `mesure` car quasi-constants par réseau, 60 car x 114M lignes sinon) + `mesure` (grain = une ligne par mesure individuelle, FK vers `parametre`/`commune`/`reseau`, contrainte d'unicité `(referenceprel, parametre_id, code_insee)`, colonne `annee` = année de l'archive Hub'Eau source, index composite `(parametre_id, annee)`, `valeur_libelle` = `rqana` brut quand il porte un qualificatif que le nombre perd : `<0,5`, `>100`, `N.M.`).
...
- **Cache d'agrégation `commune_valeur`** (table dérivée, une ligne par `(parametre_id, annee, code_insee)` : `valeur_somme` / `nb_mesures` / `derniere_mesure` / `nb_non_conformes`). Recalculée intégralement à chaque seed depuis `mesure` (`seed_db.populate_commune_valeur`, un `GROUP BY` par `parametre_id` pour tenir la RAM du box WSL).
- **Paramètres à taux de non-conformité** (E. coli aujourd'hui) : quand un paramètre déclare un `seuil_conformite` dans `PARAMETERS` de `seed_db.py`, `populate_commune_valeur` remplit `commune_valeur.nb_non_conformes` = nombre de mesures avec `valeur > seuil` (`COUNT(*) FILTER`). L'endpoint d'agrégation renvoie alors `taux_non_conformite` (= `SUM(nb_non_conformes) * 100 / SUM(nb_mesures)`, en %) et `nb_non_conformes`, et le frontend affiche ce taux au lieu de la moyenne (une seule mesure très élevée ne doit pas écraser une commune dont l'eau est par ailleurs conforme ; pour E. coli toute détection est une non-conformité). `nb_non_conformes` est NULL pour tous les autres paramètres, `taux_non_conformite` aussi, et l'UI garde la moyenne pour eux. Ajouter un paramètre à seuil = une clé `seuil_conformite` côté pipeline + `compliance: { threshold, unit }` dans `parameters.ts` + des classes en %, zéro migration.
- **`seed_db.py --recompute-cache`** : reconstruit uniquement `commune_valeur` depuis les lignes `mesure` déjà en base (pas d'archive Hub'Eau, pas de COPY, juste le `GROUP BY` par `parametre_id`). Sert à déployer une colonne dérivée ajoutée à `commune_valeur` (ex. `nb_non_conformes`) ou un `seuil_conformite` modifié sur une base déjà seedée (local ou VM Oracle via le tunnel SSH), sans re-seeder les 10 ans. `mesure` reste la source de vérité et la source du panneau de détail ; `commune_valeur` n'existe que parce qu'un `GROUP BY` live sur ~114M lignes `mesure` mettait 8 s (24 s pour `/annees`) sur ce box (35 Go de table, 8 Go de RAM). Après cache : ~0,4 s au niveau commune, instantané ailleurs.
...
- Pas de state management lourd. L'état (paramètre, unité, thème, niveau, année, commune sélectionnée, leaderboard) vit dans `App.tsx` et descend en props. Tout sauf le thème est aussi sérialisé dans la query string (`urlState.ts`) pour des permaliens partageables.
...
- `CommuneMesures.tsx` : bloc partagé résumé + tableau/graphe des relevés d'une commune (pour le paramètre + année actifs). Résumé (moyenne ou taux de non-conformité, nb mesures, dernière date) calculé **côté client** à partir des relevés déjà fetchés (pas un second appel API), donc générique à n'importe quel paramètre. `variant="compact"` (dans `CommunePanel`, carte, ~600px) : colonnes date/valeur/réseau. `variant="full"` (dans `Leaderboard`) : ajoute une colonne **Classe** (chip `classifyValue` par ligne, même sémantique que carte/résumé, "?" pour un `<LQ`) et **Distributeur**. La limite réglementaire (`REGULATORY_LIMITS[param]`) s'affiche en légende sous le résumé et en ligne de référence sur le graphe, dans les deux variants. **`conclusionprel` (verdict ARS du prélèvement entier, `mesure.conclusion`) n'est plus affiché** : il jugeait tous les paramètres du prélèvement d'un coup, trompeur sur une ligne mono-paramètre. Colonne encore renvoyée par l'API, gardée pour un futur popover prélèvement.
- `CommunePanel.tsx` / `CommuneDetail.tsx` : le panneau carte est une coquille (Paper flex-column côté carte + en-tête nom/fermer) autour de `CommuneDetail variant="compact"`. `CommuneDetail` porte les 2 onglets (`Tabs`) : "<paramètre actif>" = `CommuneMesures` sur `parameterId`, "Bulletin" = `Bulletin initialExpanded={parameterId}`. Ouvre sur l'onglet paramètre. Côté carte, `onParameterChange` remonte jusqu'à `App.handleParameterChange` : ouvrir une autre ligne du Bulletin change `parameterId` (donc la carte, la légende, l'onglet). Le même `CommuneDetail` sert le drill du Leaderboard (`variant="full"`, sans `onParameterChange` car la carte est masquée).
- `Leaderboard.tsx` : vue **plein écran** (Paper `inset:0`, ouverte depuis la TopBar). Navigateur à deux niveaux : DataGrid de toutes les communes classées (reste monté quand on drille, donc tri/filtre/page préservés), clic sur une ligne -> `drilledCode` -> `CommuneDetail variant="full"` par-dessus (onglets paramètre/Bulletin, paramètre du classement en cours), avec fil d'Ariane retour + bouton "Voir sur la carte" (`handleDrillIntoCommune` : cale le niveau carte sur commune, ferme le classement). `TimelineSlider` en pied de vue (le slider carte est masqué par le plein écran). La commune drillée est remontée à `App` (`onDrilledCommuneChange`) et re-seedée (`initialDrilledCommune`) pour le permalien `?classement=1&entite=` ; le `zoneCode` (fil d'Ariane) reste interne, non partagé.
...
2. Base Postgres (`seed_db.py`) : 13 régions, 96 départements, 1241 EPCI, 34 746 communes, ~24 442 réseaux (avec `distributeur`/`maitre_ouvrage`), **~120M mesures** (`mesure`, ~37 Go) sur les **29 paramètres** de `PARAMETERS` (les 24 d'origine + nitrites/ammonium/selenium/nickel/pfas ; pfas quasi vide avant 2023, surveillance obligatoire depuis janvier 2026), plus le cache `commune_valeur`. `mesure.valeur_libelle` peuplé depuis `rqana` pour les lignes `<LQ` (surtout contaminants trace).
3. API FastAPI opérationnelle : `/parametres`, `/annees`, `/aggregation/{commune|epci|departement|region}?parametre=code[&annee=YYYY]` (chaque ligne porte aussi `taux_non_conformite` / `nb_non_conformes`, non-null seulement pour les paramètres à seuil comme E. coli), `/aggregation/{niveau}/{code}/communes` (détail d'une zone), `/communes/{code_insee}`, `/communes/{code_insee}/mesures?parametre=code[&annee=YYYY]`, `/communes/{code_insee}/bulletin?annee=YYYY` (une ligne par paramètre mesuré : nb_mesures/minimum/maximum/moyenne/derniere_valeur/derniere_date/nb_non_conformes, lus directement depuis `mesure` car scoped à une commune ; `avg()` pour la moyenne (identique au `SUM/SUM` du choropleth sur cette commune), `DISTINCT ON` pour la dernière valeur), `/health`, Swagger sur `/api/docs`.
...
Pour ajouter un nouveau paramètre (ex. plomb) : voir la discussion archivée sur les indicateurs candidats. La bactériologie (E. coli) a maintenant son mode d'agrégation dédié (taux de non-conformité, voir "Paramètres à taux de non-conformité") ; le réutiliser pour tout paramètre quasi binaire présence/absence.

## Dernier lot de features avant le volet cybersécurité

L'app est jugée mature (septembre 2026). Chantiers restants prévus avant de basculer sur la sécurité :

1. **Tendances par commune (pas seulement un curseur).** La timeline expose 10 ans de données mais n'en affiche qu'une année à la fois. Ajouter, dans le panneau commune (`CommuneMesures`), un sparkline du paramètre actif sur toutes les années seedées, avec un signal quand la tendance va dans le mauvais sens (hausse pour un paramètre à seuil). Données déjà là côté API (`/aggregation/commune?parametre=&annee=` par année, ou un nouvel endpoint série temporelle par commune si le N+1 devient trop coûteux). Se marie avec le Bulletin.
2. ~~Permaliens (état dans l'URL).~~ **Fait (septembre 2026)** : `urlState.ts` + wiring dans `App.tsx`. Query string, pas de router. Clés : `parametre`, `annee`, `unite` (seulement si != défaut du paramètre), `niveau` (seulement si != commune), `classement` (leaderboard ouvert), `entite` (un code : sans `classement` = sélection carte ; avec `classement` = commune drillée dans le leaderboard). `App` parse l'URL 1x au montage (`readUrlState`, valeurs validées, fallback sinon), `replaceState` à chaque changement d'état (`writeUrlState`). Restauration : sélection carte via `loadEntityIndex(niveau)` dans un effet (a besoin de la bbox), avec un `restorePendingRef` qui retient la 1re écriture tant que l'entité n'est pas résolue ; commune du leaderboard passée telle quelle en `initialDrilledCommune` (juste un code, pas d'async). Le fil d'Ariane du leaderboard (zone -> commune pour niveau > commune) n'est PAS restauré : le lien saute direct sur la commune, le retour va à la liste. Pas dans l'URL : mode clair/sombre (localStorage), centre/zoom carte.
3. **Page "À propos / sources / méthode"** (= section 6 de la refonte visuelle, voir plus bas). Doit documenter publiquement : sources (Hub'Eau DIS, IGN Admin Express, INSEE COG), le caveat année en cours partielle, et surtout **le choix d'attribution commune = "tous les prélèvements des réseaux (UDI) qui desservent la commune"** via `DIS_COM_UDI`, pas seulement les prélèvements dont le point de mesure est physiquement dans la commune (le `code_commune` de l'API Hub'Eau). Sans ça, l'écart de comptage (ex. Lattes : 73 relevés E. coli côté Ondine vs 14 sur `resultats_dis?code_commune=`) sera pris pour un bug. Le lien "Source : Hub'Eau" du panneau utilise déjà `code_reseau=` pour retomber sur le même jeu.

Écartés volontairement : le zoom multi-niveaux sur la carte elle-même (utile mais pas bloquant, l'API le permet déjà), et le score composite "meilleure eau de France" (paramètres non commensurables sur un axe bon/mauvais unique, cf. la discussion "eau agressive" et le paragraphe score composite ci-dessous).
...
**Piste 2 implémentée pour E. coli (septembre 2026).** Mécanisme générique `seuil_conformite` -> `commune_valeur.nb_non_conformes` -> `taux_non_conformite` dans la réponse d'agrégation, côté frontend `ParameterDef.compliance` (voir la section "Cache d'agrégation" et "Paramètres à taux de non-conformité"). Seul E. coli l'utilise pour l'instant (seuil 0, toute détection = non-conformité). Étendre à pH/nitrates/pesticides = ajouter `seuil_conformite` côté pipeline + `compliance` côté frontend + des classes en %, puis `seed_db.py --recompute-cache`. Question ouverte pour pH (divergent : hors norme des deux côtés, 6,5 et 9) : le mécanisme actuel ne teste que `valeur > seuil`, il faudrait un intervalle (`< bas OR > haut`) pour ces paramètres.
...
**`Leaderboard.tsx` est un navigateur de données à deux niveaux** (septembre 2026) : plein écran depuis la TopBar, classement des communes -> drill sur une commune (`CommuneDetail variant="full"` : mêmes onglets paramètre/Bulletin que le panneau carte), grille de classement conservée montée (état tri/filtre préservé), `TimelineSlider` en pied. Le classement reste par paramètre, mais le drill montre tous les paramètres de la commune.

**Idée de classement composite, toutes communes sur tous les paramètres.** Une seule note par commune combinant les paramètres présents, pour "quelle commune a la meilleure eau, tout confondu".
...
.venv/bin/python src/seed_db.py --recompute-cache      # reconstruit seulement commune_valeur depuis mesure (pas d'archive) ; pour déployer une colonne dérivée / un seuil_conformite modifié sans re-seeder
...
`seed_db.py` est idempotent (TRUNCATE + reload), donc relancer la commande en cas de problème ne duplique rien. Le job traite ~6M lignes (29 paramètres) par année en ~2-3 min : compter ~N fois ça pour `--year` avec N années (les années sont traitées séquentiellement, une seule archive RESULT résidente à la fois). Le lancer en arrière-plan plutôt que d'attendre en bloquant le terminal.

Pour un changement qui ne touche que la couche dérivée (nouvelle colonne `commune_valeur`, `seuil_conformite` ajouté/modifié), pas besoin de re-seeder : la table `mesure` de la VM a déjà les 10 ans. Appliquer la migration (redéploiement backend, `alembic upgrade head` au démarrage) puis, via le même tunnel, `DATABASE_URL=...@localhost:5433/ondine .venv/bin/python src/seed_db.py --recompute-cache` (de l'ordre de la minute, garde-fou anti-OOM identique au seed).
...
- **Effets `MapView` gardés sur `getSource(SOURCE_ID) === undefined`** : au montage ils tournent avant le `"load"` MapLibre. Ceux qui dépendent de `parameterId`/`level`/`annee` sont rattrapés par le handler `"load"` (qui rejoue `*Ref.current`). L'effet de sélection/`fitBounds`, lui, dépend d'un état `mapReady` (flippé dans le handler `"load"`) et pas seulement du garde : sinon un permalien qui restaure `selectedEntity` avant le chargement de la carte ne zoome jamais sur la commune.
...
- Un paramètre monitoré = une entrée dans `PARAMETERS` (pipeline `seed_db.py`) + une entrée dans `PARAMETERS` + un `PARAMETER_ICONS` (frontend `TopBar.tsx`) + le groupe de menu dans `PARAMETER_GROUPS`, + `REGULATORY_LIMITS[id]` s'il a un seuil réglementaire. Rien d'autre à toucher pour que l'API/carte/légende/panneau/classement le supportent. Vérifier le code SANDRE et son unité de report dans les données brutes avant (voir `filter_parameter`, qui fait échouer le seed sur unité inattendue).
...
- Codes SANDRE des paramètres : **1345** = dureté/Titre Hydrotimétrique (°f), **1302** = pH (unité pH), **1340** = nitrates en NO3 (mg/L), **1339** = nitrites en NO2, **1335** = ammonium en NH4, **1385** = sélénium, **1386** = nickel, **8847** = somme des 20 PFAS (µg/L). Ne pas confondre 1345 et 1340 (erreur présente dans une version antérieure de ce fichier). La liste complète (29 paramètres) est dans `PARAMETERS` de `pipeline/src/seed_db.py`, avec le pendant `PARAMETERS` de `frontend/src/parameters.ts` (échelle de couleur, `unitsHelp`, groupe de menu) + une entrée dans `REGULATORY_LIMITS` (`parameters.ts`) pour ceux à seuil. `filter_parameter` **assère l'unité attendue** : un mauvais code SANDRE fait échouer le seed (rollback propre) au lieu de produire une carte plausible mais fausse.
- Les données Hub'Eau sont à la granularité **UDI (unité de distribution)**, pas directement commune. Une UDI peut couvrir plusieurs communes ou une commune plusieurs UDI. L'agrégation par défaut est une **moyenne** des mesures de la commune (lisse les écarts entre réseaux plutôt que d'en privilégier un), aussi bien côté CSV pipeline que côté API (`SUM/SUM` sur `commune_valeur`, équivalent d'un `AVG` poolé). **Exception : E. coli** est agrégé en **taux de non-conformité** (part des prélèvements avec détection), pas en moyenne, car toute détection est une non-conformité et un comptage isolé très élevé fausserait la moyenne. Voir "Paramètres à taux de non-conformité".
...
- **Une seule statistique dans toute l'app : la moyenne poolée.** Choropleth (`SUM/SUM` sur `commune_valeur`, rollup propre commune -> EPCI -> dept -> région), colonne Moyenne + chip Classe du Bulletin (`avg()` sur `mesure` de la commune, chiffre identique), résumé de `CommuneMesures` (moyenne des relevés fetchés) : les trois affichent et classent sur le même nombre, pas de divergence de classe près d'une borne. Le Bulletin montre en plus Plage (min-max) et Dernier pour ne pas cacher la dispersion. Exception : paramètre à seuil (E. coli), classé au **taux de non-conformité** (`nb_non_conformes / nb_mesures`) partout. Médiane écartée en septembre 2026 : elle créait des incohérences carte/bulletin et, pour les toxiques chroniques, la moyenne est la bonne métrique d'exposition (dose moyenne annuelle).
- Classes de couleur : **séquentielles** (une extrémité = préoccupant) pour dureté et nitrates, **divergentes** (deux extrémités = préoccupantes, centre = neutre/bon) pour le pH et la conductivité. Respecter cette distinction pour tout nouveau paramètre plutôt que réutiliser une rampe par défaut. Les métaux/contaminants trace (plomb, arsenic, sélénium, nickel, THM, bisphénol, PFAS) partagent un schéma 20/40/70/100 % du seuil réglementaire.
- **Conformité par prélèvement**: `conclusionprel` (`mesure.conclusion`) est le verdict de **tout le prélèvement** (tous paramètres du `referenceprel`), pas de la ligne mono-paramètre affichée. Ne jamais l'afficher comme un jugement sur la valeur d'un paramètre (une ligne de dureté "non conforme" à cause d'E. coli dans le même prélèvement). La conformité par ligne s'exprime via le chip de classe (`classifyValue`, les classes extrêmes des paramètres à seuil sont déjà libellées "hors norme"). Hub'Eau n'a **pas** de flag de conformité par paramètre.
